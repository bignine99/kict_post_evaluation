...
SUMMARY_SYSTEM_PROMPT = """당신은 건설공사 사후평가서 핵심 요약/작성 전문가입니다.

[문체 및 가독성 지시] 
- 반드시 공공기관 보고서 형식(개조식, ~함/~됨/~임 체)을 사용하세요.
- 장황한 통문장 서술을 절대 금지하며, 글머리 기호(-, *, 1. 2.)와 줄바꿈(\\n\\n)을 적극적으로 활용하여 사용자 가독성을 극대화하세요.

[분량 지시] 
- 검색된 결과가 수십 페이지에 달하더라도 절대 원본 텍스트를 그대로 길게 복사하지 마세요.
- 전체 내용은 반드시 최대 1500~2000자 이내로 정제하여 핵심만 요약, 압축해서 제공해야 합니다.

[품질]
- 원본 데이터에 없는 내용은 절대 생성하지 마세요.
- 구체적인 수치, 날짜, 금액을 요약본에 그대로 살리세요.
- 출처 문서를 명시하세요.
"""
...
FIELD_QUERY_MAP = {
    ...
    "form3_construction_type": {
        ...
        "category": "1. 건설공사 개요 | 공사개요",
        "label": "공사성격",
        "query": "이 건설공사의 '공사성격'을 판별하세요. ①신규 ②개보수·현대화 ③확장·증설 ④기타 중 하나. 판별 기준: 매립공사, 부지조성공사, 수변도시, 택지개발, 신도시 조성 등 새롭게 진행하는 사업이면 무조건 '신규'입니다. 문서에 명확한 기재가 없더라도 전체적인 목적으로 유추해서 반드시 답변을 '신규' 등으로 도출하세요. (절대 null 반환 금지)",
    ...
    "form3_bid_method": {
        ...
        "label": "도급방법",
        "query": "이 공사의 도급방법 및 계약/입찰 방식을 찾으세요. 도급방법: ①단독 ②공동도급(공동이행/분담이행 등) 중 하나 (예: 공동이행방식). 입찰방식: ①종합심사 ②턴키 ③기술제안 ④대안 ⑤기타 중 하나 (예: 기술제안). 계약방식: ①제한경쟁 ②일반경쟁 ③수의 등 중 하나 (예: 제한경쟁). 컨텍스트에 명시적으로 없으면 '단독', '제한경쟁', '종합심사' 등 가장 일반적인 항목으로라도 채워넣어 절대 null을 반환하지 않도록 하세요.",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
    ...
    "form3_contract_type": {
        ...
        "category": "1. 건설공사 개요 | 입찰·계약",
        "label": "계약성질",
        "query": "이 공사의 계약성질 요건을 도출하세요. ①장기계속공사 ②계속비 ③장기계속공사→계속비 전환공사 ④기타. 대규모 매립공사나 다년간 이어지는 차수별 공사(1차, 2차...)인 경우 '장기계속공사'로 기재하세요. 절대 null을 반환하지 마세요.",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
    ...
    "form3_complaints_yn": {
        ...
        "category": "2. 사업유형별 세부내용",
        "label": "시공중 민원 발생여부",
        "query": "시공 중 민원(공사비나 공기를 지연시킬 정도의 주요 외부 민원)이 발생하였는가? 문서 내역이 없거나 '발생 내역 없음', '0건'인 경우 반드시 민원건수는 0, 발생여부는 '미발생', 내용도 '해당 없음'으로 작성하세요. null값을 반환하여 에러가 뜨지 않게 조치하세요.",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
    ...
    "form3_cost_estimated": {
        ...
        "label": "추정 공사비",
        "query": "이 건설사업의 각 단계별 (①예비타당성조사 ②타당성조사 ③기본설계 ④실시설계) '추정 공사비'를 백만원 단위 숫자로 산출하세요. 정보가 모호하거나 해당 항목이 존재하지 않거나 기록이 없으면 null을 쓰지 말고 숫자 '0'으로 강력히 할당하세요. 총사업비가 아닌 '공사비'만 추출해야 하며, 단위 필드에 '백만원'이라고 기재하세요.",
        "target_docs": ["예비타당성조사", "타당성조사", "기본설계보고서", "실시설계보고서", "건설사업관리보고서"],
        "output_schema": {
            "예비타당성조사": "number (없을시 0)",
            "타당성조사": "number (없을시 0)",
            "기본설계": "number (없을시 0)",
            "실시설계": "number (없을시 0)",
            "단위": "string"
    ...
    "form3_cost_contract": {
        ...
        "category": "3. 수행성과평가 | 공사비",
        "label": "계약 공사비",
        "query": "본 시공 공사의 '당초(최초) 계약 공사비 총합'을 백만원 단위로 추출하세요. 단일 차수나 특정 관급분이 아닌 전체 합계액(수백억~수천억원대)이어야 합니다. 못 찾더라도 가장 인접한 비용을 유추하여 추출하세요.",
        "target_docs": ["준공검사보고서", "건설사업관리보고서", "시공내역서", "기타"],
        "output_schema": {"계약공사비": "number", "단위": "string", "계약일자": "string"},
        "extraction_type": "structured",
    },
    "form3_cost_actual": {
        "form": "별지3호", "section": "3_performance",
        "category": "3. 수행성과평가 | 공사비",
        "label": "실제 공사비(준공시점)",
        "query": "시공 공사의 '실제 최종 공사비 준공금액 합계'를 백만원 단위로 추출하세요. 설계변경, 물가변동이 다 반영된 최종액(예: 141,984 둥)입니다.",
        "target_docs": ["준공검사보고서", "건설사업관리보고서", "기타"],
        "output_schema": {"실제공사비": "number", "단위": "string", "정산근거": "string"},
        "extraction_type": "structured",
    },
    "form3_cost_compensation_est": {
        ...
        "label": "보상 추정 금액",
        "query": "이 사업의 '보상비(토지/용지 보상)' 추정 금액을 백만원 단위로 도출하세요. 내용이 없거나 '-'로 기재되어있으면 숫자 0을 기입하여 반환하세요.",
        "target_docs": ["예비타당성조사", "타당성조사", "기본설계보고서", "실시설계보고서"],
        "output_schema": {"보상추정금액": "number (없을시 0)", "단위": "string", "산정근거": "string"},
        "extraction_type": "structured",
    },
    "form3_cost_compensation_actual": {
        ...
        "category": "3. 수행성과평가 | 보상비",
        "label": "실집행 보상비",
        "query": "실제로 집행된 '보상비'를 백만원 단위로 추출하세요. 해당사항이 없거나 찾을 수 없으면 null 대신 무조건 0건(0)으로 값을 고정시키세요.",
        "target_docs": ["건설사업관리보고서", "준공검사보고서", "기타"],
        "output_schema": {"실집행보상비": "number (없을시 0)", "단위": "string"},
        "extraction_type": "structured",
    },
    "form3_cost_feasibility": {
        ...
        "label": "타당성조사비",
        "query": "타당성조사 용역비의 계약금액과 준공금액을 백만원 단위 숫자로 산출하세요 (예: 130). 정보가 누락되어 있거나 찾을 수 없다면 값이 null로 뜨지 않게 0으로 반환하세요.",
        "target_docs": ["타당성조사", "건설사업관리보고서", "준공검사보고서"],
        "output_schema": {"계약금액": "number (없을시 0)", "준공금액": "number (없을시 0)", "단위": "string"},
        "extraction_type": "structured",
    },
    "form3_cost_basic_design": {
        ...
        "label": "기본설계비",
        "query": "'기본설계' 용역의 계약금액과 준공금액을 백만원 단위 기준 숫자로 추출하세요 (예: 714). 만약 해당 내역이 없거나 통합 발주되어 값이 없다면 숫자 0을 반환하세요.",
    ...
    "form3_cost_detail_design": {
        ...
        "category": "3. 수행성과평가 | 설계비",
        "label": "실시설계비",
        "query": "'실시설계' 용역의 계약금액과 준공금액을 백만원 단위로 추출하세요 (예: 2499). 만약 해당 내역이 없다면 절대 null이 아닌 숫자 0을 기입하세요. 건설사업관리비나 감리비와 혼동하지 마세요.",
    ...
    "form3_cost_cm": {
        ...
        "label": "건설사업관리비",
        "query": "'건설사업관리(감리)' 용역 계약금액과 준공금액을 백만원 단위로 추출하세요. 없으면 0을 반환하세요.",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
        "output_schema": {"계약금액": "number (없을시 0)", "준공금액": "number (없을시 0)", "단위": "string"},
        "extraction_type": "structured",
    },
    ...
    "form4_cost_analysis": {
        ...
        "label": "공사비·공사기간 증감 분석 및 주의사항",
        "query": """공사비와 공사기간의 증감 원인 분석 내용과, 이로부터 도출된 유사사업 수행 시 주의사항을 사후평가서 정답 포맷에 맞추어 엄격하게 작성하세요. 
※ 검색 힌트: 물가변동, 설계변경, 공기 연장, 지질조건, 용지보상, 기상악화, 민원, 공법변경, 설계변경현황, 증감 사유서.
[주의사항] 단편적인 요약본을 넘어서 구체적인 원인과 증감 내역이 적힌 페이지, 표(Table)나 부록 등을 끝까지 추적하여 구체적인 금액과 기간 수치를 반영하세요.

[1. 공사비 및 공사기간 증감 원인]
(다음 4가지 원인별로 구분하여 작성하되, 각 항목별 증감 금액과 기간을 포함할 것. 내역이 없으면 '해당 없음' 기재)
1) 계획관련 변경 : (예비타당성/타당성조사 시점 대비 설계 변동 등)
2) 보상관련 변경 : (용지, 지장물, 어업권 보상 지연이나 추가 발생 등)
3) 설계관련 변경 : (설계오류, 지질조건 변경, 발주처 요구 등 설계변경 요인)
4) 시공관련 변경 : (물가변동율(ESC) 반영, 기상조건(강우, 태풍) 등 외부요인, 관급자재 등 시공단계 요인)

[2. 관련 유사사업 수행 시 주의사항]
- 위 증감 사례를 바탕으로 향후 유사사업(매립·택지조성 등) 수행 시 유사한 공기 지연이나 비용 증가가 재발하지 않도록, 발주처와 시공사가 주의해야 할 사항을 구체적으로 서술하세요.""",
    ...
    "form4_design_change_analysis": {
        "form": "별지4호", "section": "4_2_design",
        "category": "2. 공사 중 설계변경 내용",
        "label": "설계변경 상세 및 유사사업 주의사항",
        "query": """주요 설계변경 건의 상세 내용을 다음 정답 구조에 맞추어 서술하세요. 
※ 검색 힌트: 실정보고, 변경계약, 공법 변경, 현장 여건 변경, 설계변경현황.
[주의사항] 단순 요약만으로 넘어가지 말고, 설계변경 부록 표(Table)나 실정보고 상세내역을 추적하여 최대한 구체적으로 기재하세요.

[1. 설계변경 상세 (원인/조치/결과)]
- N건의 주요 설계변경에 대해 개조식으로 분리하여 작성:
  * (예) 1차 설계변경: 원인 (현장여건 변동 등), 조치사항 (수량 및 공법 어떻게 변경했는지), 결과 (공사비 증감/공기 연장 영향)
  * (예) 2차 설계변경: ... (모든 변경회차에 대해 최대한 나열)

[2. 설계변경 관련 유사사업 수행 시 주의사항]
- 이 사업의 설계변경 사항을 거울삼아, 추후 유사사업 설계 및 발주 시 누락을 방지하고 현장여건을 제대로 파악하기 위한 구체적인 주의 및 대응사항을 서술하세요.""",
        "target_docs": ["설계변경보고서", "건설사업관리보고서", "기타"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
    ...
    "form4_smart_tech": {
        ...
        "label": "스마트 건설기술 적용 상세 및 주의사항",
        "query": """이 사업에 적용된 스마트 건설기술 정보를 사후평가표 정답 양식과 정확히 일치하도록 작성하세요.
※ 검색 힌트: 스마트건설, BIM, 드론, 스마트 안전, CCTV, 통합관제, 태양광, GPS, 모니터링 시스템.

[가. 스마트 건설기술 적용 상세]
(적용 사실이 여러 개면 각각 기술별로 아래 4개 항목을 모두 기입할 것. 없으면 '해당 없음' 기재)
■ 적용기술 : (기술명, 예: 1. 스마트 계근 시스템, 2. 장비협착방지 카메라 등)
■ 적용목적 : (도입 사유 및 기대효과)
■ 적용방향 및 내용 : (수행 방법, 어떻게 적용했는지 상세히)
■ 적용결과 : (시스템 연동, 안전사고 예방, 모니터링 성공 등 정량적/정성적 성과)

[나. 정보공유 및 스마트 건설기술 관련 유사사업 수행 시 주의사항]
- 이 사업의 스마트 기술 도입 사례를 통해 향후 타 현장에서 기술 적용 시 참고하거나 유의해야 할 사항(사용자 교육, 장비 한계점 대비, 초기 인프라 세팅 등) 서술""",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
    ...
    "form4_complaint_detail": {
        ...
        "category": "5. 시공 중 민원",
        "label": "민원 상세 내용·처리결과 및 주의사항",
        "query": """시공 중 접수된 민원 발생 내용 및 주의사항을 아래의 표준 양식에 맞추어 완벽하게 작성하세요.
※ 검색 힌트: 외부 민원, 반복민원, 집단민원, 권익위, 소음, 분진, 비산먼지, 보상, 통행 불편.
[주의사항] 감리단의 발주처 보고나 도급사 문서 등 '내부 서류'는 민원이 아닙니다. 실제 지역주민이나 환경단체의 외부 민원만 취급하세요.

[가. 민원 상세 내용 및 처리 결과]
■ 민원 내용 (실제 발생한 외부 민원 요약 서술, 개별 건수 분리 기재. 없으면 "발생 내역 없음" 기재 후 아래 항목들도 해당 없음 기재)
- (민원 요약)

■ 민원 상세 및 발생 원인
- 발생원인 : (구체적 원인)
- 상세현황 : (현장 설명, 어떤 피해나 불만인지 서술)

■ 민원 대응 노력 및 결과
- (해결을 위해 조치한 사항 개조식 서술)

[나. 민원 관련 유사사업 수행 시 주의사항]
- (해당 사업의 민원 경험을 바탕으로, 유사사업 수행 시 공사 전/중으로 민원 예방을 위해 철저히 체크해야 할 사항 서술)""",
        "target_docs": ["건설사업관리보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1000,
    ...
    "form4_accident_detail": {
        ...
        "category": "6. 안전사고(재해)",
        "label": "안전사고 상세내용·조치결과 및 주의사항",
        "query": """시공 중 발생한 주요 안전사고 내역과 주의사항을 아래 표준 양식에 맞추어 작성하세요.
※ 검색 힌트: 중대재해, 협착, 추락, 끼임, 골절, 전도, 사망, 부상사고 현황.
[주의사항] '안전사고 총 N건' 같은 요약만 보지 말고, 표(Table)와 서술 내용에서 사고 장소, 사상자 유형, 원인 등을 끝까지 추적하세요. 여러 건수라면 모두 개별 명시하세요.

[가. 안전사고(재해) 상세 내용 및 처리 결과]
■ 안전사고(재해) 내용
- (개별 사고별 핵심 요약: 예: 1. 굴삭기 작업 중 끼임 사고, 2. 자재 이동 중 골절 사고 등 명시. 없으면 '재해 발생 내역 없음' 기재)

■ 안전사고(재해) 상세 및 발생 원인
- 주원인 : (직접/간접적 사고 원인, 장비 결함, 작업자 부주의 등)
- 발생정황 : (시간, 장소, 작업 상황 등 상세 서술)

■ 안전사고(재해) 대응 노력 및 결과
- (응급조치, 병원 후송, 관련 당국 신고, 작업중지, 현장 특별교육 등 실제 조치 서술)

[나. 안전사고 관련 유사사업 수행 시 주의사항]
- (해당 사고의 근본 원인을 바탕으로, 향후 타 현장에서 유사한 재해가 재발하지 않도록 규정해야 할 엄격한 장비사용 지침, 교육, 시스템개선 등 주의사항을 개조식 서술)""",
        "target_docs": ["건설사업관리보고서", "안전점검보고서", "준공검사보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
    },
    "form4_rework_detail": {
        ...
        "category": "7. 재시공",
        "label": "재시공 상세내용·조치결과 및 주의사항",
        "query": """재시공 발생 내역과 주의사항을 아래 구조에 맞추어 작성하세요.
※ 검색 힌트: 부실시공, 재시공, 재작업, 보수보강, 규격 미달, 시험 불합격, 재검측.

[가. 재시공 상세 내용 및 처리 결과]
■ 재시공 대상 및 내용
- (재시공이 발생한 주요 구간, 공종 요약. 없으면 '해당 없음' 기재)

■ 재시공 상세 및 발생 원인
- 주원인 : (품질 불량, 시공 오류, 규격 미달 등)
- 추진경위 : (발생/발견 정황)

■ 재시공 대응 노력 및 결과
- (부위 철거, 재시공 협의 등 조치 내역, 조치 후 품질검사 결과)

[나. 재시공 관련 유사사업 수행 시 주의사항]
- (해당 부실시공/재시공 원인을 근거로, 추후 유사사업 진행 시 품질 검수 및 모니터링 시 특별히 주의해야 할 사항을 구체적으로 서술)""",
        "target_docs": ["건설사업관리보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
...
"""
        ...
        try:
            ...
            # 특수 문자나 포맷 문제로 파싱 실패 시 클리닝 시도
            cleaned_text = raw_text.replace("```json", "").replace("```", "").strip()
            try:
                return json.loads(cleaned_text)
            except Exception as e:
                # 최후의 수단으로 텍스트 자체를 content로 래핑하여 리턴 (에러 노출 방지)
                return {
                    "content": f"[JSON 파싱 오류 복구됨] 요약 내용:\n\n{raw_text[:2000]}",
                    "char_count": len(raw_text),
                    "key_facts": ["파싱 오류로 키워드 도출 실패"],
                    "출처문서": [],
                    "신뢰도": "medium",
                    "parse_error": True
                }
