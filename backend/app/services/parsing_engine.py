...
FIELD_QUERY_MAP = {
    ...
    "form4_cost_analysis": {
        ...
        "query": """공사비와 공사기간의 증감 원인을 분석하세요. 
※ 검색 키워드 강화를 위한 힌트: 물가변동, 설계변경, 공기 연장, 지질조건 변경, 용지보상, 기상악화, 민원, 공법변경, 단가조정, 전체공사 증감 대비표, 증감 사유서.
[주의사항] 단편적인 요약본만 보지 말고, 구체적인 원인과 증감 내역이 적힌 페이지, 표(Table)나 부록 등을 끝까지 추적하세요.

다음 6가지 원인별로 구분하여 작성: ①계획관련(예비타당성/타당성조사 단계 추정 부정확 등) ②보상관련(용지보상 지연/추가보상 등) ③설계관련(설계변경, 설계오류, 지질조건 변경 등) ④시공관련(공법변경, 물가변동, 자재가격 상승 등) ⑤외부요인(기상악화, 코로나, 민원, 관련기관 협의 지연 등) ⑥기타. 각 원인별로 증감 금액(백만원)과 증감 기간(개월)을 구체적으로 기술하세요.""",
    ...
    "form4_design_change_analysis": {
        ...
        "query": """주요 설계변경 건의 상세 내용을 서술하세요. 
※ 검색 키워드 강화를 위한 힌트: 실정보고, 도면 변경, 수량 증감, 공법 변경, 현장 여건 변경, 시방서 변경, 구조 변경.
[주의사항] 단순 요약만으로 '상세 정보가 없다'고 단정짓지 마세요. 설계변경 사유서나 부록의 표(Table)를 샅샅이 찾아 최대한 구체적인 항목을 기재하세요.

각 주요 설계변경에 대해: ①원인(왜 변경이 필요했는가? 현장여건 변경, 발주처 요구, 관련법규 변경 등) ②조치사항(어떤 설계를 어떻게 변경했는가? 공법, 구조, 수량 등) ③결과(변경 후 공사비/공기 영향, 품질 변화 등)를 서술하세요. 가능하면 사진이나 도면 참조 위치도 기재하세요.""",
        "target_docs": ["설계변경보고서", "건설사업관리보고서", "기타"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 2000,
    ...
    "form4_smart_tech": {
        ...
        "query": """적용한 스마트 건설기술을 상세히 서술하세요. 
※ 검색 키워드 강화를 위한 힌트: 스마트건설, BIM, 3D 모델링, 드론 측량, 무인기, 센서, IoT, 지능형 CCTV, 로봇, 모니터링 시스템, 프리팹, 모듈러, AI 예측.
[주의사항] 적용 사실을 찾을 때 본문 텍스트 외에 이미지 캡션이나 표 내용 등도 철저히 추적하세요.

각 기술별로: ①기술명 ②적용목적(왜 적용했는가) ③적용방법(어디에 어떻게 적용했는가) ④적용결과(효과, 성과)를 기술하세요. 스마트 건설기술 예시: BIM, 드론 측량, IoT 센서, CCTV 원격감시, AI 기반 품질관리 등. 적용 사실이 없으면 '해당없음'으로 작성하세요.""",
        "target_docs": ["건설사업관리보고서", "준공검사보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
...
※ 검색 키워드 강화를 위한 힌트: 반복민원, 집단민원, 국민신문고, 소음, 진동, 비산먼지, 보상구구, 가처분, 공사중지가처분, 통행 불편, 주민설명회, 갈등관리.
[주의사항] 발주처-시공사 간의 안전지시 문서, 단순 실정보고, 협조요청 등 내부 공문 수발신 내역은 "민원"이 아닙니다. 실제 외부(지역주민, 환경단체, 관련기관 등)로부터 제기된 민원 사실만을 민원으로 판단하세요. '민원 처리 대장' 표(Table)가 존재하면 그 내용을 샅샅이 반영하세요.

[가. 민원 상세 내용 및 처리 결과]
■ 민원 내용 (실제 발생한 외부 민원 요약 서술, 개별 건수 분리 기재)
- (없으면 "시공 중 접수 및 처리된 주요 외부 민원 내역 없음" 기재)
...
※ 검색 키워드 강화를 위한 힌트: 협착, 추락, 장비 전도, 끼임, 절단, 부딪힘, 골절, 사망, 부상, 안전사고 원인 및 결과 분석.
[주의사항] 단편적으로 '중대재해 N건 발생'이라는 요약 문구만 보고 "구체적인 내역이 없다"며 넘어가는 환각을 엄격히 금지합니다. '안전사고 원인 및 결과 분석' 목차나 구체적인 사상자, 장비 결함 등의 발생 상세 내역을 표(Table)나 이미지 캡션 등을 포함하여 끝까지 추적하세요. 발생된 모든 사고 건수 각각에 대해 개별 상세 내용을 반드시 기재하세요. (없을 경우에만 없다고 기재)

[가. 안전사고(재해) 상세 내용 및 처리 결과]
■ 안전사고(재해) 내용 (개별 사고별로 모두 기재)
- (사고 핵심 요약: 예: PBD 장비 작업 중 협착사고사고, 굴삭기 장비 전도 사고, 배사관 손가락 끼임사고 등 구체적 명시)

■ 안전사고(재해) 상세 및 발생 원인
- 주원인 : (직접적/간접적 사고 발생 원인, 작업자 부주의 등)
- 발생정황 : (언제 어디서 어떤 작업 중 발생했는지 상세 서술)
(※ 문서 원본에 사고 현장 사진이나 설명이 있다면 관련 내용을 상세 묘사할 것)

■ 안전사고(재해) 대응 노력 및 결과
- (사고발생에 대한 현장 조사, 공사중지 지시, 종합정보망 신고, 재발방지 교육 실시 등 후속 조치사항 개조식 서술)""",
        "target_docs": ["건설사업관리보고서", "안전관리계획서", "안전점검보고서", "준공검사보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
    },
...
※ 검색 키워드 강화를 위한 힌트: 재시공, 철거, 보수보강, 부실시공, 균열, 파손, 침하, 규격 미달, 시험 불합격, 재작업.
[주의사항] 단순하게 '재시공 N건 발생'이라는 요약 텍스트만 보고 "상세내역 없음"이라 환각하지 마세요. 반드시 표(Table)나 이미지 캡션을 끝까지 추적해서 개별 건별 조치사항을 모두 발굴하세요. 여러 번의 재시공이 있다면 각 건마다 분리하여 기술하세요.

[가. 재시공 상세 내용 및 처리 결과]
■ 재시공 대상 및 내용 (건별 분리)
- (재시공이 발생한 주요 구간, 수량, 공종 요약)

■ 재시공 상세 및 발생 원인 (건별 분리)
- 주원인 : (설계 시공 지질 등 핵심 원인, 규격 미달 등)
- 추진경위 : (발생 및 발견 정황 서술)
(※ 문서 내 관련 현장 조치 전/후 사진이 존재하면 [사진: 조치 전후 전경] 형태로 묘사)

■ 재시공 대응 노력 및 결과 (건별 분리)
- (재시공 방안 협의 및 조치, 철거 후 재시공 내역)
- (조치 후 품질검사/강도 시험 등 결과)""",
        "target_docs": ["건설사업관리보고서"],
        "output_schema": {},
        "extraction_type": "narrative",
        "target_length": 1500,
